        ...
        if (chance < 1f && Config.DropRateModifier == 0)
        {
            foreach (MonsterDrop drop in Drops)
            {
                if (!GameItem.DataSheetByName.TryGetValue(drop.Name, out var item) || 
                    ...
                    (hitter.本期特权 == 0 && Grade != MonsterGradeType.Boss && item.Type != ItemType.可用药剂 && Compute.CalculateProbability(0.5f)) || 
                    (hitter.本期特权 == 3 && Grade != MonsterGradeType.Boss && item.Type != ItemType.可用药剂 && Compute.CalculateProbability(0.25f)))
                {
                    continue;
                }
