...
public sealed class MonsterObject : MapObject
{
    ...
    public List<MonItemInfo> Drops => Info.Drops;
    ...
    private void DropItems(PlayerObject hitter)
    {
        ...
        if (ratio < 1f && Config.DropRateModifier == 0)
        {
            foreach (MonItemInfo drop in Drops)
            {
                ...
                if (!GameItem.DataSheetByName.TryGetValue(drop.Name, out var item) || 
                    (hitter.CurrentPrivilege == 0 && Grade != MonsterGradeType.Boss && item.Type != ItemType.可用药剂 && Compute.CalculateProbability(0.5f)) || 
                    (hitter.CurrentPrivilege == 3 && Grade != MonsterGradeType.Boss && item.Type != ItemType.可用药剂 && Compute.CalculateProbability(0.25f)))
                {
                    continue;
                }
        ...
        if (ratio < 1f && Config.DropRateModifier == 1 && Config.CurrentVersion >= 1)
        {
            int num15 = 0;
            int num16 = 0;
            int num17 = 0;
